...
def migration_path_name(migration_name)
  return Dir.glob(Rails.root.join(ActiveRecord::Migrator.migrations_paths.first, "*#{migration_name}.rb")).first
end
...
def rollback_to_previous_of!(migration_file)
  ...
  if previous_version_index == all_migrations.length - 1
    previous_version_file_name = all_migrations[all_migrations.length - 2]
  else
    previous_version_file_name = all_migrations[previous_version_index - 1]
  end
