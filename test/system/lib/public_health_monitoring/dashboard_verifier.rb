...
class PublicHealthMonitoringDashboardVerifier < ApplicationSystemTestCase
  ...
  def verify_patients_on_dashboard(jurisdiction_id, verify_scope=false)
    ...
    verify_patients_under_tab(jurisdiction, verify_scope, 'symptomatic', patients.symptomatic.where(isolation: false))
    verify_patients_under_tab(jurisdiction, verify_scope, 'non-reporting', patients.non_reporting.where(isolation: false))
    verify_patients_under_tab(jurisdiction, verify_scope, 'asymptomatic', patients.asymptomatic.where(isolation: false))
    verify_patients_under_tab(jurisdiction, verify_scope, 'pui', patients.under_investigation.where(isolation: false))
    ...
    verify_workflow_count('Exposure Monitoring', patients.where(isolation: false).count)
    verify_workflow_count('Isolation Monitoring', patients.where(isolation: true).count)
    verify_patients_under_tab(jurisdiction, verify_scope, 'requiring-review', patients.isolation_requiring_review.where(isolation: true))
    verify_patients_under_tab(jurisdiction, verify_scope, 'non-reporting', patients.isolation_non_reporting.where(isolation: true))
    verify_patients_under_tab(jurisdiction, verify_scope, 'reporting', patients.isolation_reporting.where(isolation: true))
