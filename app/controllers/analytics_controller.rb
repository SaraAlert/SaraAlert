...
class AnalyticsController < ApplicationController
  ...
  def epi_stats # rubocop:todo Metrics/AbcSize
    ...
    [dates.last].each_with_index do |d, i|
      symptomatic_patient_count_by_state_and_day << { day: d }
      total_patient_count_by_state_and_day << { day: d }
      states.each do |state|
        count1 = Assessment.joins(:patient).where('assessments.created_at::date = ?', d)
                           .where(symptomatic: true, 'patients.monitored_address_state' => state).count
        symptomatic_patient_count_by_state_and_day[i][state] = count1
        count2 = Assessment.joins(:patient).where('assessments.created_at::date = ?', d)
                           .where('patients.monitored_address_state' => state).count
