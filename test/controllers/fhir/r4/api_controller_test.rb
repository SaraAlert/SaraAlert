...
class ApiControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'SYSTEM FLOW: should be 406 when bad accept header via show' do
    get '/fhir/r4/Patient/1', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'foo/bar' }
    assert_response :not_acceptable
  end

  test 'SYSTEM FLOW: should be unauthorized via show write only' do
    get '/fhir/r4/Patient/1', headers: { 'Authorization': "Bearer #{@system_token_w.token}", 'Accept': 'application/fhir+json' }
    assert_response :forbidden
  end
  ...
  test 'SYSTEM FLOW: should get observation via show' do
    get '/fhir/r4/Observation/1001', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
  ...
  test 'SYSTEM FLOW: should get QuestionnaireResponse via show' do
    get '/fhir/r4/QuestionnaireResponse/1001', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 1001, json_response['id']
  ...
  test 'SYSTEM FLOW: should be bad request via show' do
    get '/fhir/r4/FooBar/1', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :bad_request
  end

  test 'SYSTEM FLOW: should be forbidden via show' do
    get '/fhir/r4/Patient/9', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :forbidden
  end
  ...
  test 'SYSTEM FLOW: should be unauthorized via create read only' do
    post '/fhir/r4/Patient', params: @patient_1.to_json, headers: { 'Authorization': "Bearer #{@system_token_r.token}" }
    assert_response :forbidden
  end
  ...
  test 'SYSTEM FLOW: should be unauthorized via update read only' do
    put '/fhir/r4/Patient/1', params: @patient_1.to_json, headers: { 'Authorization': "Bearer #{@system_token_r.token}" }
    assert_response :forbidden
  end
  ...
  test 'SYSTEM FLOW: should be unauthorized via search write only' do
    get '/fhir/r4/Patient?family=Kirlin44', headers: { 'Authorization': "Bearer #{@system_token_w.token}", 'Accept': 'application/fhir+json' }
    assert_response :forbidden
  end

  test 'SYSTEM FLOW: should find Observations for a Patient via search' do
    get '/fhir/r4/Observation?subject=Patient/1', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
  ...
  test 'SYSTEM FLOW: should find QuestionnaireResponses for a Patient via search' do
    get '/fhir/r4/QuestionnaireResponse?subject=Patient/1', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
  ...
  test 'SYSTEM FLOW: should find no Observations for an invalid Patient via search' do
    get '/fhir/r4/Observation?subject=Patient/blah', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
  ...
  test 'SYSTEM FLOW: should find Patient via search by _id' do
    get '/fhir/r4/Patient?_id=1', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
  ...
  test 'SYSTEM FLOW: should find Patient via search on existing family' do
    get '/fhir/r4/Patient?family=Kirlin44', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
    assert_equal 1, json_response['total']
  ...
  test 'SYSTEM FLOW: should find no Patients via search on non-existing family' do
    get '/fhir/r4/Patient?family=foo', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
    assert_equal 0, json_response['total']
  ...
  test 'SYSTEM FLOW: should find Patient via search on given' do
    get '/fhir/r4/Patient?given=Chris32', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
    assert_equal 1, json_response['total']
  ...
  test 'SYSTEM FLOW: should find Patient via search on telecom' do
    get '/fhir/r4/Patient?telecom=%28555%29%20555-0141', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
    assert_equal 1, json_response['total']
  ...
  test 'SYSTEM FLOW: should find Patient via search on email' do
    get '/fhir/r4/Patient?email=grazyna%40example.com', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
    assert_equal 1, json_response['total']
  ...
  test 'SYSTEM FLOW: should get Bundle via search without params' do
    get '/fhir/r4/Patient', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
  ...
  test 'SYSTEM FLOW: should be bad request via search' do
    get '/fhir/r4/FooBar?email=grazyna%40example.com', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :bad_request
  end
  ...
  test 'SYSTEM FLOW: should be unauthorized via all write only' do
    get '/fhir/r4/Patient/1/$everything', headers: { 'Authorization': "Bearer #{@system_token_w.token}", 'Accept': 'application/fhir+json' }
    assert_response :forbidden
  end

  test 'SYSTEM FLOW: should get Bundle via all' do
    get '/fhir/r4/Patient/1/$everything', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
  ...
  test 'SYSTEM FLOW: should get patients with count as 100 via search' do
    get '/fhir/r4/Patient?_count=100', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
  ...
  test 'SYSTEM FLOW: should only get summary with count as 0 via search' do
    get '/fhir/r4/Patient?_count=0', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal 'Bundle', json_response['resourceType']
    assert_nil json_response['link']
  ...
  test 'SYSTEM FLOW: should be forbidden via all' do
    get '/fhir/r4/Patient/9/$everything', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :forbidden
  end
  ...
  test 'SYSTEM FLOW: should get CapabilityStatement authorized via capability_statement' do
    get '/fhir/r4/metadata', headers: { 'Authorization': "Bearer #{@system_token_rw.token}", 'Accept': 'application/fhir+json' }
    assert_response :ok
    json_response = JSON.parse(response.body)
    assert_equal ADMIN_OPTIONS['version'], json_response['software']['version']
  end
