const mockTranslations = {
  eng: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro:
            "'Welcome to the Sara Alert system, we will be sending your daily reports for %{name} to this phone number. For privacy and more info, visit example.com.'",
  ...
  spa: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro:
            "'Bienvenido al sistema Sara Alert, le enviaremos sus reportes diarios para %{name} a este número de teléfono. Para privacidad y más información, visite example.com.'",
  ...
  fra: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro:
            "'Bienvenue sur le système d''Alerte Sara, nous vous enverrons vos rapports quotidiens pour %{name} à ce numéro de téléphone. Pour la confidentialité et plus d''informations, visitez example.com.'",
  ...
  som: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro:
            "'Ku soo dhowow nidaamka Sara Alert, waxaan ku soo diri doonnaa warbixinadaada maalinlaha ah ee loogu talagalay %{name} lambarkan telefoon. Xogta gaarka ah iyo macluumaad dheeri ah, booqo example.com.'",
  ...
  kor: {
    ...
    html: {
      ...
      email: {
        shared: {
          ...
          report: '일일 보고서',
          footer:
            '이 이메일에 답장하거나, 이 이메일을 전달하거나, 이 링크를 공유하지 마십시오. 이 메시지는 Sara Alert 시스템에 의해 자동으로 생성되었으며 고유하고 귀하만을 위한 것입니다. 이러한 알림을 중단하고 싶거나 실수라고 생각되면 지역 공중 보건부에 문의하십시오. 개인 정보 보호 및 더 자세한 정보를 원하시면 example.com를 방문하십시오.',
        },
        enrollment: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro:
            'Sara Alert 시스템에 오신 것을 환영합니다. 이 전화번호로 %{name}님의 일일 보고서를 보내드리겠습니다. 개인 정보 보호 및 더 자세한 정보를 원하시면 example.com를 방문하십시오.',
  ...
  vie: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro:
            'Chào mừng bạn đến với hệ thống Sara Alert, chúng tôi sẽ gửi các báo cáo hàng ngày của bạn cho %{name} tới số điện thoại này. Để biết về quyền riêng tư và biết thêm thông tin, hãy truy cập example.com.',
  ...
  rus: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro:
            'Добро пожаловать в систему Sara Alert. Мы будем отправлятьежедневные опросы для %{name} на этот номер телефона.  Чтобы подробнее узнать о программе, системе конфиденциальности, а также получить дополнительную информацию, посетите example.com.',
          reminder: 'This is the Sara Alert system reminding you to please reply to our daily-report messages.',
          name: '%{name}',
          daily:
  ...
  ara: {
    ...
    html: {
      ...
      email: {
        shared: {
          ...
          footer:
            'تم إنشاؤه بواسطة نظام Sara Alert وهو فريد ومخصص لك فقط. إذا كنت ترغب في التوقف عن تلقي هذه الإخطارات أو تعتقد أنه كان خطأ، يرجى الاتصال بقسم الصحة العامة المحلي. للخصوصية ومزيد من المعلومات، قم بزيارة example.com.',
        },
        enrollment: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro: 'مرحبًا بك في نظام Sara Alert، سنرسل لك تقاريرك اليومية عن %{name} إلى رقم هذا الهاتف. للخصوصية ومزيد من المعلومات، قم بزيارة example.com.',
  ...
  zho: {
    ...
    html: {
      ...
      email: {
        shared: {
          ...
          report: '每日报告',
          footer:
            '不要回复此电子邮件，转发此电子邮件或分享此链接。此消息是由Sara Alert系统自动生成的，并且是唯一的，仅供您使用。如果您希望停止接收这些通知，或者认为这是一个错误，请联系您当地的公共卫生部门。 有关隐私和更多信息，请访问example.com。',
        },
        enrollment: {
    ...
    twilio: {
      ...
      sms: {
        prompt: {
          intro: '欢迎使用Sara Alert系统。我们会将%{name}每日报告发送到这个电话号码。有关保护隐私和更多信息，请访问example.com。',
