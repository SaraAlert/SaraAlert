...
class ClosePatientsJob < ApplicationJob
  ...
  def perform(*_args)
    ...
    eligible.each do |patient|
      ...
      # If the patient was enrolled already past their monitoring period based on their LDE, specify special reason for closure
      if (patient.last_date_of_exposure.beginning_of_day + ADMIN_OPTIONS['monitoring_period_days'].days) < patient.created_at.beginning_of_day
        ...
        # If the patient was enrolled oon their last day of monitoring based on their LDE, specify special reason for closure
      ...
      if patient.save! && patient.email.present?
        PatientMailer.closed_email(patient).deliver_later if patient.self_reporter_or_proxy?
      end
