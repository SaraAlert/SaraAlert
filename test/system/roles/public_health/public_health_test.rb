...
class PublicHealthTest < ApplicationSystemTestCase
  ...
  test 'bulk edit from exposure to isolation' do
    @@public_health_test_helper.bulk_edit_case_status('state1_epi', ['patient_3'], :exposure, 'symptomatic', 'Confirmed', 'Continue Monitoring in Isolation Workflow')
  end

  test 'bulk edit from isolation to exposure' do
    @@public_health_test_helper.bulk_edit_case_status('state1_epi', [], :isolation, 'records requiring review', 'Unknown', false)
  end
