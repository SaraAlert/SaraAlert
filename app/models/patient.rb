...
class Patient < ApplicationRecord
  ...
  scope :enrolled_in_time_frame, lambda { |time_frame|
    ...
    when 'Last 7 Days'
      where('patients.created_at >= ? AND patients.created_at < ?', 7.days.ago.to_date.to_datetime, DateTime.now.utc.beginning_of_day)
    when 'Last 14 Days'
      where('patients.created_at >= ? AND patients.created_at < ?', 14.days.ago.to_date.to_datetime, DateTime.now.utc.beginning_of_day)
    when 'Total'
      all
    else
      none
  ...
  scope :closed_in_time_frame, lambda { |time_frame|
    ...
    when 'Last 7 Days'
      where('patients.closed_at >= ? AND patients.closed_at < ?', 7.days.ago.to_date.to_datetime, DateTime.now.utc.beginning_of_day)
    when 'Last 14 Days'
      where('patients.closed_at >= ? AND patients.closed_at < ?', 14.days.ago.to_date.to_datetime, DateTime.now.utc.beginning_of_day)
    when 'Total'
      all
    else
      none
  ...
  scope :ten_day_quarantine_candidates, lambda {
    where(purged: false, monitoring: true, isolation: false, continuous_exposure: false)
      ...
      .where_assoc_exists(:assessments) do
        ...
        where(
          'CAST(CONVERT_TZ(assessments.created_at, "UTC", patients.time_zone) AS DATE)'\
          '  BETWEEN DATE_ADD(last_date_of_exposure, INTERVAL 10 DAY)'\
          '  AND DATE_ADD(last_date_of_exposure, INTERVAL 13 DAY)'
        )
      end
      .where(
        'DATE_ADD(last_date_of_exposure, INTERVAL 10 DAY) '\
        '  <= CAST(CONVERT_TZ(?, "UTC", patients.time_zone) AS DATE)',
        Time.now.utc
      )
  }
  ...
  scope :seven_day_quarantine_candidates, lambda {
    where(purged: false, monitoring: true, isolation: false, continuous_exposure: false)
      ...
      .where_assoc_exists(:assessments) do
        ...
        where(
          'CAST(CONVERT_TZ(assessments.created_at, "UTC", patients.time_zone) AS DATE)'\
          ' BETWEEN DATE_ADD(last_date_of_exposure, INTERVAL 7 DAY)'\
          ' AND DATE_ADD(last_date_of_exposure, INTERVAL 9 DAY)'
        )
      end
      .where(
        'DATE_ADD(last_date_of_exposure, INTERVAL 7 DAY) '\
        '  <= CAST(CONVERT_TZ(?, "UTC", patients.time_zone) AS DATE)',
        Time.now.utc
      )
