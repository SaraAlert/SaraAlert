...
class ClosePatientsJob < ApplicationJob
  ...
  def perform(*_args)
    ...
    eligible.each do |patient|
      ...
      local_created_at_date = patient.created_at.getlocal(patient.address_timezone_offset).to_date
      if !patient.last_date_of_exposure.nil? &&
         ((patient.last_date_of_exposure.beginning_of_day + ADMIN_OPTIONS['monitoring_period_days'].days) < local_created_at_date)
