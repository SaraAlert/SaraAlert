class UpdateVaccineNames < ActiveRecord::Migration[6.1]
  ...
  def down
    ...
    ActiveRecord::Base.connection.raw_connection.prepare(Arel.sql("UPDATE user_export_presets SET config = REPLACE(config, 'Pfizer-BioNTech COVID-19 Vaccine (Non-US tradename: COMIRNATY)', 'Pfizer-BioNTech COVID-19 Vaccine') WHERE config REGEXP 'Pfizer'")).execute
    ...
    ActiveRecord::Base.connection.raw_connection.prepare(Arel.sql("UPDATE user_filters SET contents = REPLACE(contents, 'Pfizer-BioNTech COVID-19 Vaccine (Non-US tradename: COMIRNATY)', 'Pfizer-BioNTech COVID-19 Vaccine') WHERE contents REGEXP 'Pfizer'")).execute
  end
end
