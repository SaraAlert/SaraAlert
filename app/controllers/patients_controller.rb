...
class PatientsController < ApplicationController
  ...
  def create
    ...
    patient.responder = if params.permit(:responder_id)[:responder_id]
                          ...
                          if current_user.viewable_patients.where(monitoring: true).responder_for_number(patient[:primary_telephone])&.exists?
                            current_user.viewable_patients.where(monitoring: true).responder_for_number(patient[:primary_telephone]).first
                          end
                        elsif patient[:preferred_contact_method] == 'E-mailed Web Link'
                          if current_user.viewable_patients.where(monitoring: true).responder_for_email(patient[:email])&.exists?
                            current_user.viewable_patients.where(monitoring: true).responder_for_email(patient[:email]).first
                          end
                        end
