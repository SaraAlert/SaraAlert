...
class UserTest < ActiveSupport::TestCase
  ...
  test 'user status with manual lock of "" and auto_lock_reason' do
    user = create(:user, locked_at: 2.days.ago, force_password_change: 1, manual_lock_reason: '')

    assert_equal(user.status, '')
    assert_equal(user.lock_reason, '')
    assert_equal(user.auto_lock_reason, 'Temporary password expired')
  end

  test 'user status with locked_at timestamp, no auto_lock_reason and nil manual_lock_reason' do
