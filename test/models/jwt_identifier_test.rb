...
class JwtIdentifierTest < ActiveSupport::TestCase
  ...
  test 'purge eligible scope' do
    ...
    jwt_identifier = create(:jwt_identifier, value: "a", expiration_date: Time.now - 2.minutes, application_id: @oauth_app.id)
    assert_equal(1, JwtIdentifier.purge_eligible.count) 

    # JWT that expired now SHOULD be purge eligible
    jwt_identifier = create(:jwt_identifier, value: "b", expiration_date: Time.now, application_id: @oauth_app.id)
    ...
    jwt_identifier = create(:jwt_identifier, value: "c", expiration_date: Time.now + 30.seconds, application_id: @oauth_app.id)
    assert_equal(0, JwtIdentifier.purge_eligible.count) 

    # JWT that expires in 5 minutes should NOT be purge eligible
    jwt_identifier = create(:jwt_identifier, value: "d", expiration_date: Time.now + 1.day, application_id: @oauth_app.id)
    assert_equal(0, JwtIdentifier.purge_eligible.count) 
  end
end
