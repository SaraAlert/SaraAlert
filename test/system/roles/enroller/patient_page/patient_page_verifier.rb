...
class EnrollerPatientPageVerifier < ApplicationSystemTestCase
  ...
  def verify_monitoree_info(monitoree, is_epi=false)
    ...
    @@enroller_form_steps.steps.each do |enrollment_step, enrollment_fields|
  ...
  def verify_group_member_info(existing_monitoree, new_monitoree, is_epi=false)
    find('#patient-info-header').click if is_epi
    verify_enrollment_step(new_monitoree['identification'], @@enroller_form_steps.steps[:identification])
    verify_enrollment_step(existing_monitoree['address'], @@enroller_form_steps.steps[:address])
    verify_enrollment_step(existing_monitoree['contact_info'], @@enroller_form_steps.steps[:contact_info])
    verify_enrollment_step(existing_monitoree['arrival_info'], @@enroller_form_steps.steps[:arrival_info])
    verify_enrollment_step(existing_monitoree['additional_planned_travel'], @@enroller_form_steps.steps[:additional_planned_travel])
    verify_enrollment_step(existing_monitoree['potential_exposure_info'], @@enroller_form_steps.steps[:potential_exposure_info])
  end
