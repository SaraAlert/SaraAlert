  ...
  def up
    ...
    ActiveRecord::Base.transaction do
      Jurisdiction.find_each do |jurisdiction|
        ...
        puts "JURISDICTION IDENTIFIER HASH COLLISION FOR: #{jurisdiction[:path]}" if Jurisdiction.where('BINARY unique_identifier = ?', new_unique_identifier).any?
  ...
  def down
    ...
    ActiveRecord::Base.transaction do
      JurisdictionLookup.find_each do |jurisdiction_lookup|
        jurisdiction = Jurisdiction.where('BINARY unique_identifier = ?', jurisdiction_lookup.new_unique_identifier).first
