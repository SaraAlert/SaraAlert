...
class PublicHealthController < ApplicationController
  ...
  def closed_patients
    ...
    render json: filter_sort_paginate(params, current_user.viewable_patients.monitoring_closed)
  end
  ...
  def sort(params, data)
    ...
    params[:order].each do |_num, val|
      if val['column'] == '0' # Name
        sorted = sorted.order(last_name: val['dir'].to_sym).order(first_name: val['dir'].to_sym)
      elsif val['column'] == '1' # Jurisdiction
        sorted = sorted.includes(:jurisdiction).order('jurisdictions.name ' + val['dir'])
      elsif val['column'] == '2' # ID
        sorted = sorted.order(user_defined_id_statelocal: val['dir'].to_sym)
      elsif val['column'] == '3' # Sex
        sorted = sorted.order(sex: val['dir'].to_sym)
      elsif val['column'] == '4' # DOB
        sorted = sorted.order(date_of_birth: val['dir'].to_sym)
      elsif val['column'] == '5' # End of Monitoring
        sorted = sorted.order(last_date_of_exposure: val['dir'].to_sym)
      elsif val['column'] == '6' # Risk
        risk_categories = ['High', 'Medium', 'Low', 'No Identified Risk']
        sorted = sorted.order_by_risk(val['dir'] == 'asc' ? risk_categories : risk_categories.reverse)
      elsif val['column'] == '7' # Plan
        sorted = sorted.order(monitoring_plan: val['dir'].to_sym)
      elsif val['column'] == '8' # Latest Report
        sorted = sorted.includes(:latest_assessment).order('assessments.created_at ' + val['dir'])
      end
    end
    sorted
  end
