...
function getMountedWrapper() {
  return mount(<ApplyToHousehold household_members={householdMembers} current_user={mockUser1} jurisdiction_paths={mockJurisdictionPaths} handleApplyHouseholdChange={handleApplyHouseholdChangeMock} handleApplyHouseholdIdsChange={handleApplyHouseholdIdsChangeMock} />);
}
...
describe('ApplyToHousehold', () => {
  ...
  it('Sorting household members table maintains selected and disabled rows', () => {
    ...
    const wrapper = mount(<ApplyToHousehold household_members={householdMembers} current_user={mockUser1} jurisdiction_paths={mockJurisdictionPaths} handleApplyHouseholdChange={handleApplyHouseholdChangeMock} handleApplyHouseholdIdsChange={handleApplyHouseholdIdsChangeMock} />);
    ...
    wrapper
      ...
      .forEach((row, index) => {
        ...
        if (disabledRows.includes(index)) {
          expect(row.find('td').at(1).find('div').text()).toEqual(nameFormatterAlt(householdMembers[Number(index)]));
        } else {
    ...
    wrapper
      ...
      .forEach((row, index) => {
        ...
        expect(row.find('input').prop('disabled')).toEqual(disabledRows.includes(index));
        if (disabledRows.includes(index)) {
          expect(row.find('td').at(1).find('div').text()).toEqual(nameFormatterAlt(sortedHouseholdMembers[Number(index)]));
        } else {
          expect(row.find('td').at(1).find('a').text()).toEqual(nameFormatterAlt(sortedHouseholdMembers[Number(index)]));
        }
      });
