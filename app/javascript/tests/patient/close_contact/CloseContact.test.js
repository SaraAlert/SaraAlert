...
function getShallowWrapper(closeContact) {
  return shallow(<CloseContact authenticity_token={mockToken} patient={mockPatient1} close_contact={closeContact} can_enroll_patient_close_contacts={true} assigned_users={ASSIGNED_USERS} />);
}
  ...
  it('Properly renders the modal if button is clicked', () => {
    ...
    expect(
      emptyCCWrapper
        ...
        .at(2)
        .find(Form.Label)
        .containsMatchingElement(<InfoTooltip />)
    ).toBeTruthy();
